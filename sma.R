# BDA400 Assignment 5: Technical Analysis using R (Development Phase) sma.R, Simple Moving Average (SMA)
#
# AI HELP DECLARATION:
...
#
# WHAT THIS DOES: the Simple Moving Average gives equal weight to every point in a trailing window of `period` values, like smoothing short-term noise so the underlying trend is easier to see.
#
# Keep in mind ON OUTPUT LENGTH: following the assignment's pseudocode exactly, this function returns a vector of length (length(data) - period + 1), i.e. it is NOT padded with NA at the start. sma_values[1] is the average of the first `period` points, sma_values[2] slides the window forward by one, etc.

sma <- function(data, period) {
  # Frankly, guard: you cannot average a window that is larger than the data you have
...
# TESTS (run this file directly, e.g. `Rscript sma.R`, or source() it in RStudio, every value below was produced by actually running this code)
if (sys.nframe() == 0) {
  ...
  print((10 + 12 + 15) / 3)
  # CHECKED OUTPUT: [1] 12.33333, matches sma_values[1] above

  cat("\n=== Edge case: period equal to full data length ===\n")
  print(sma(c(5, 10, 15), period = 3))
  # CHECKED OUTPUT: [1] 10 (single window covering all 3 points)
