# BDA400 Assignment 5: Technical Analysis using R (Development Phase) linreg.R, Linear Regression (trailing-window)
#
# AI HELP DECLARATION:
...
# `regressionOffset` points back from the most recent observation (so you can ask "what did the trend look like a few bars ago"). Returns the slope, intercept, and the line's predicted values over that window.
#
# BUG FOUND DURING TESTING, WINDOW SIZE:
# The assignment's pseudocode finds the window like this:
#   What i mean is, start_index = max(1, n - regressionLength + regressionOffset) end_index = min(n, n - regressionOffset)
# For example, applied literally, the resulting window is NOT `regressionLength` points long: its length works out to (regressionLength - 2*regressionOffset
# + 1), so with regressionOffset = 0 it is one point too LONG, and it gets shorter still as regressionOffset grows, confirmed by direct testing
# That is, (see the explanation document for the details). Since the guideline is explicitly named `regressionLength`, I corrected the index math below so the window is always exactly `regressionLength` points, ending `regressionOffset` points back from the end of the source data, then checked the corrected output against R's own lm() on an identical window, which matched exactly (slope and intercept equal to at least 3 decimal places).
...
linreg <- function(regressionSource, regressionLength, regressionOffset) {
  ...
  # Guard: an offset that reaches (or exceeds) the window length leaves no room for an actual window
  if (regressionOffset >= regressionLength) {
    stop("regressionOffset must be less than regressionLength")
  }

  # CORRECTED window calculation: end `regressionOffset` points back from the most recent observation, then walk back exactly `regressionLength` points
...
if (sys.nframe() == 0) {
  ...
  # CHECKED OUTPUT:
  # I mean, offset= 0 -> length = 5 values: 16 17 18 19 20 offset= 1 -> length = 5 values: 15 16 17 18 19 offset= 2 -> length = 5 values: 14 15 16 17 18
  # (always length 5, correctly sliding back by `offset` -- the literal pseudocode instead gives lengths 6, 4, 2 for these same calls)
  ...
  # CHECKED OUTPUT: slope = 0.965 intercept = 113.923 predicted_values: 114.888 115.853 116.818 117.783 118.748
